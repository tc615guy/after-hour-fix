  ...
  const handleImportCSVPreview = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        setCsvMapping({
          name: find('name','customer'),
          phone: find('phone','mobile','cell'),
          email: find('email','e-mail'),
          datetime: find('appointment date/time','appointment datetime','date/time','datetime'),
          date: find('appointmentdate','start date','date'),
          startTime: find('starttimelocal','start time'),
          endTime: find('endtimelocal','end time'),
          street: find('serviceaddress','address','street'),
          city: find('city'),
          state: find('state'),
          zip: find('zip','zipcode','postal'),
          appointmentId: find('appointmentid','apptid','id'),
          jobNumber: find('jobnumber','job no'),
          status: find('status'),
          value: find('value','amount','price','total'),
          campaignSource: find('campaignsource','source'),
          notes: find('notes','memo','desc'),
        })
      } catch (error: any) {
              ...
              <div className="mb-6 p-4 border rounded-lg bg-gray-50">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                  {[
                    ...
                    { key: 'datetime', label: 'Appointment Date/Time' },
