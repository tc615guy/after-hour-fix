...
const ENERGETIC_MALE_VOICE = {
  provider: 'cartesia',
  voiceId: 'a0e99841-438c-4a64-b679-ae501e7d6091', // Helpful Man
  model: 'sonic-english',
  language: 'en'
}
...
function getPromptForTrade(trade: string, businessName: string): string {
  const tradeSpecific = {
    plumbing: {
      ...
      examples: "leaky faucet, clogged drain, water heater issue"
    },
    electrical: {
      greeting: `Hey! ${businessName} here. What can I help you with?`,
      issue: "What's the electrical issue?",
      examples: "outlet not working, circuit breaker tripping, lighting problem"
    },
    hvac: {
      greeting: `Hey! ${businessName} here. What's going on?`,
      issue: "What's the issue with your heating or cooling?",
      examples: "AC not cooling, heater not working, strange noises"
    }
  }

  const config = tradeSpecific[trade.toLowerCase() as keyof typeof tradeSpecific] || tradeSpecific.plumbing

  return `You are the energetic, helpful AI receptionist for ${businessName}. You're upbeat and ready to help, but keep a mellow, professional tone.
...
async function updateAssistant(name: string, id: string) {
  ...
  try {
    ...
    console.log(`   Voice: Energetic Male (${ENERGETIC_MALE_VOICE.voiceId})`)
    console.log(`   First message: ${greeting}`)
    console.log(`   Requires: Full address confirmation + Phone confirmation`)
...
async function main() {
  console.log('🔧 Updating All Assistants to Energetic Male Voice\n')
  console.log('Changes to apply:')
  console.log('  • Voice: Energetic Male (a0e99841-438c-4a64-b679-ae501e7d6091)')
  console.log('  • Require full numerical address')
  console.log('  • Confirm phone number back to caller')
  console.log('  • Confirm full address back to caller')
  console.log('  • Energetic but mellow, professional tone')
  console.log('')
